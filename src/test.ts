...
const brawlDir =
  process.platform == "darwin"
    ? `${process.env.HOME}/Library/Application Support/Steam/steamapps/common/Brawlhalla/Brawlhalla.app/Contents/Resources/`
...
function testWriteSWZMetadata() {
  ...
  swzWriter.writeData(
    Buffer.from("<Metadata><ModName>Test Mod</ModName></Metadata>"),
    0xdeadbeef
  );
...
function dumpSWZ() {
  ...
  for (const file of readdirSync(brawlDir)) {
    ...
    const swzReader = new SWZReader(
      readFileSync(`${brawlDir}${file}`),
      decryptionKey
    );
